    ...
    public function table() {
        ...
        foreach ($rows->result() as $row) {
            $data[] = array(
                ...
                '5'         => htmlspecialchars($row->patient_diantar),
                '6'         => !empty($row->tgl_datang) ? date('d/m/Y', strtotime($row->tgl_datang)) : '-',
                '7'         => htmlspecialchars($row->jam),
                '8'         => htmlspecialchars($row->docnumb),
                '9'         => !empty($row->docdate) ? date('d/m/Y', strtotime($row->docdate)) : '-',
            );
        }
